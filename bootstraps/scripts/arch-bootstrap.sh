...
install_pacman_packages() {
  ...
  for PACKAGE in $BASIC_PACKAGES; do
    ...
    if [[ -e "$DOWNLOAD_DIR/$FILE" ]];then
      debug "using cached file $DOWNLOAD_DIR/$FILE"
    else
...
preserved_packages() {
  local DEST=$1 DOWNLOAD_DIR=$2
  if [[ -d $DOWNLOAD_DIR ]];then
    mkdir -p ${DEST}/var/cache/pacman/pkg
    cp ${DOWNLOAD_DIR}/* ${DEST}/var/cache/pacman/pkg/
  fi
}
...
preserve_packages() {
  local DEST=$1 DOWNLOAD_DIR=$2
  cp ${DEST}/var/cache/pacman/pkg/* ${DOWNLOAD_DIR}/
}
...
main() {
  ...
  preserved_packages "$DEST" "$DOWNLOAD_DIR"
  install_packages "$ARCH" "$DEST" "${BASIC_PACKAGES[*]} ${EXTRA_PACKAGES[*]}"
  configure_pacman "$DEST" "$ARCH" # Pacman must be re-configured
  [[ -n "$PRESERVE_DOWNLOAD_DIR" ]] && preserve_packages "$DEST" "$DOWNLOAD_DIR" || rm -rf "$DOWNLOAD_DIR"
